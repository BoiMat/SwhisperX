...
def save_word_timestamps(result, audio_file_path, output_dir="word_timestamps"):
    """
    Save word-level timestamps from a WhisperX result to a CSV file.

    Parameters:
        result (dict): WhisperX output, where result["segments"][…]["words"] is a list of
                       {"start": float, "end": float, "word": str}.
        audio_file_path (str): Path to the original audio file, used to name the output.
        output_dir (str): Where to save the TSV. Defaults to "word_timestamps".
    """
    ...
    with open(out_path, "w", encoding="utf-8", newline="") as f:
        ...
        for seg in result.get("segments", []):
            for w in seg.get("words", []):
                ...
                # if you have a format_time helper:
                # writer.writerow([format_time(start), format_time(end), word])
                # otherwise write raw floats:
                writer.writerow([start, end, word])

    print(f"Word-level timestamps saved to {out_path}")
